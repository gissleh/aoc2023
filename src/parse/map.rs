...
impl<P, F, TP, TF> Copy for Map<P, F, TP, TF>
where
    P: Copy,
    F: Copy,
{
...
impl<P, F, TP, TF> Clone for Map<P, F, TP, TF>
where
    P: Clone,
    F: Clone,
{
...
impl<P, TP, TV> Copy for MapValue<P, TP, TV>
where
    P: Copy,
    TV: Copy,
{
...
impl<P, TP, TV> Clone for MapValue<P, TP, TV>
where
    P: Clone,
    TV: Clone,
{
...
impl<'i, P, TP, TV> Parser<'i, TV> for MapValue<P, TP, TV>
where
    P: Parser<'i, TP>,
    TV: Copy,
{
...
impl<P, F, TP, TF> Copy for FilterMap<P, F, TP, TF>
where
    P: Copy,
    F: Copy,
{
...
impl<P, F, TP, TF> Clone for FilterMap<P, F, TP, TF>
where
    P: Clone,
    F: Clone,
{
...
mod tests {
    ...
    fn filter_map_filters_and_maps_like_a_filter_map_does() {
        ...
        assert_eq!(
            parser_1.parse(b"."),
            ParseResult::new_bad("FilterMap function returned None")
        );

        assert_eq!(
            parser_1.repeat().parse(b"123#abZ.aa"),
            ParseResult::Good(vec![1, 2, 3, 999, 100, 101, 225], b".aa")
        );
    }
