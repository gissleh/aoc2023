...
impl<'i, PB, TB, PL, TL, PR, TR> QuotedBy<PB, TB, PL, TL, PR, TR> where PB: Parser<'i, TB>, PL: Parser<'i, TL>, PR: Parser<'i, TR> {
    pub fn new(body_parser: PB, lq_parser: PL, rq_parser: PR) -> Self {
        Self { body_parser, lq_parser, rq_parser, spooky_ghost: Default::default() }
    }
}

impl<PB, TB, PL, TL, PR, TR> Copy for QuotedBy<PB, TB, PL, TL, PR, TR> where PB: Copy, PL: Copy, PR: Copy {}

impl<PB, TB, PL, TL, PR, TR> Clone for QuotedBy<PB, TB, PL, TL, PR, TR> where PB: Clone, PL: Clone, PR: Clone {
    fn clone(&self) -> Self {
        Self {
    ...
    fn parse(&self, input: &'i [u8]) -> ParseResult<'i, TB> {
        match self.lq_parser.parse(input) {
            ParseResult::Good(_, new_input) => match self.rq_parser.first_parsable_in(new_input) {
                ParseResult::Good((_, cap_pos), input_after) => match self.body_parser.parse(&new_input[..cap_pos]) {
...
impl<'i, PB, TB, PC, TC> CappedBy<PB, TB, PC, TC> where PB: Parser<'i, TB>, PC: Parser<'i, TC> {
    pub fn new(body_parser: PB, cap_parser: PC) -> Self {
        CappedBy { body_parser, cap_parser, spooky_ghost: PhantomData::default() }
    }
}

impl<PB, TB, PC, TC> Copy for CappedBy<PB, TB, PC, TC> where PB: Copy, PC: Copy {}

impl<PB, TB, PC, TC> Clone for CappedBy<PB, TB, PC, TC> where PB: Clone, PC: Clone {
    fn clone(&self) -> Self {
        Self {
            body_parser: self.body_parser.clone(),
...
impl<'i, PB, TB, PC, TC> Parser<'i, TB> for CappedBy<PB, TB, PC, TC> where PB: Parser<'i, TB>, PC: Parser<'i, TC> {
    fn parse(&self, input: &'i [u8]) -> ParseResult<'i, TB> {
