...
impl<P, F, T> Copy for Filter<P, F, T>
where
    P: Copy,
    F: Copy,
{
...
impl<P, F, T> Clone for Filter<P, F, T>
where
    P: Clone,
    F: Clone,
{
...
impl<'i, P, F, T> Parser<'i, T> for Filter<P, F, T>
where
    P: Parser<'i, T>,
    F: Fn(&T) -> bool + Copy,
{
...
impl<'i, P, T> InRange<P, T>
where
    P: Parser<'i, T>,
    T: Copy,
{
    #[inline]
    pub fn new<R>(parser: P, range: R) -> Self
    where
        R: RangeBounds<T>,
    {
        Self {
...
impl<P, T> Clone for InRange<P, T>
where
    P: Clone,
    T: Clone,
{
...
impl<'i, P, T> Parser<'i, T> for InRange<P, T>
where
    P: Parser<'i, T>,
    T: Ord + Copy,
{
    ...
    fn parse(&self, input: &'i [u8]) -> ParseResult<'i, T> {
        match self.parser.parse(input) {
            ...
            ParseResult::Bad(err) => {
                ParseResult::wrap_bad(err, "Parser failed before range was involved")
            }
        }
    }
}
...
mod tests {
    ...
    use crate::parse::{signed_int, ParseResult, Parser};
    ...
    fn only_if_onlies_and_iffs() {
        ...
        assert_eq!(
            even_parser.parse(b"one"),
            signed_int::<i32>().parse(b"one"),
            "Should return the same as just passing it through."
        );
        assert_eq!(
            even_parser.parse(b"13"),
            ParseResult::new_bad("Filter rejected parsed result")
        );
    }
